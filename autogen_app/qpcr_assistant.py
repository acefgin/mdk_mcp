...
- get_sequences(taxon, region, source, max_results)
- get_taxonomy(query)
- get_neighbors(taxon, rank, distance)
- extract_sequence_columns(sequence_data, columns, output_format)
- search_sra_studies(query, filters)

After retrieving data, summarize your findings and pass to AnalystAgent.""",
            llm_config=self.llm_config
        )
...
When you have analyzed the data and made recommendations, summarize your findings for the Coordinator.""",
        ...
        self.groupchat = GroupChat(
            ...
            messages=[],
            max_round=20,
            speaker_selection_method="round_robin"  # Simple round-robin for now
        )
