 ...
 */
export function parseFastaStats(fastaContent: string): SequenceStats {
 ...
 */
export async function filterAndSave(
  fastaContent: string,
  filter: (sequence: string, header: string) => boolean,
  outputPath: string
 ...
 */
export function summarizeAlignment(alignmentContent: string): {
  sequences: number;
  length: number;
  ...
  return {
    ...
    gapPercentage:
      (totalGaps / (sequences.length * alignmentLength)) * 100,
 ...
 */
export async function getCachedResult<T = any>(key: string): Promise<T | null> {
  const cacheDir = '/workspace/cache';
  const hash = crypto.createHash('sha256').update(key).digest('hex');
  const cachePath = path.join(cacheDir, `${hash}.json`);
...
): string | { truncated: true; preview: string; size: number } {
