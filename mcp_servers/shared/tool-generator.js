...
export class ToolFileGenerator {
     ...
     */
    async generateToolFiles(serverName, tools, outputDir) {
     ...
     */
    generateToolFile(tool, serverName) {
        const functionName = this.snakeToCamel(tool.name);
        const typeName = this.capitalize(functionName);
        // Generate input interface
        const inputInterface = this.generateTypeScriptInterface(tool.inputSchema, `${typeName}Input`);
...
`;
     ...
     */
    generateReadme(serverName, tools) {
...
`;
     ...
     */
    generateTypeScriptInterface(schema, name) {
