...
export class MCPClient {
     ...
     */
    async searchTools(query, detailLevel = 'name') {
        const results = [];
     ...
     */
    async getToolDescriptions(serverName) {
        const serverConfig = this.config.servers[serverName];
        if (!serverConfig) {
            throw new Error(`Server '${serverName}' not found in configuration`);
        }
     ...
     * @returns Tool result
     */
    async callTool(serverName, toolName, args, timeout = 30000) {
        const serverConfig = this.config.servers[serverName];
        if (!serverConfig) {
            throw new Error(`Server '${serverName}' not found in configuration`);
        }
     ...
     * @private
     */
    async fetchTools(serverName, containerName, detailLevel) {
        // Build MCP request
        const initRequest = JSON.stringify({
            jsonrpc: '2.0',
            id: 1,
 ...
 */
export async function callMCPTool(toolId, params, timeout = 30000) {
    ...
    return client.callTool(serverName, toolName, params, timeout);
}
/**
 ...
 */
function getServerScriptPath(serverName) {
